var apiUrl = 'http://localhost:3000/demo/'; // 'https://5ce07h92eh.execute-api.ap-southeast-2.amazonaws.com/demo/';
...
var cdnUrl = 'https://cdn.shotstack.io/au/v1/';
var progress = 0;
 ...
 */
function initialiseVideo(src, captions) {
    player = new Plyr('#player', {
        controls: ['play-large', 'play', 'progress', 'mute', 'volume', 'download', 'fullscreen', 'captions'],
        captions: {
            active: true,
            language: 'en',
            update: true
...
function pollVideoStatus(id) {
    $.get(apiEndpoint + '/' + id, function (response) {
        ...
        var transcriptionUrl = cdnUrl + response.data.owner + '/' + response.data.id + '/transcript.vtt';
        ...
        if (!(status === 'ready' || status === 'failed')) {
            ...
            initialiseVideo(rendition.url, transcriptionUrl);
            initialiseJson({ rendtions: [{ transformation: rendition.transformation }], transcription: { format: 'vtt' } });
            initialiseDownload(transcriptionUrl);
...
/**
 * Submit the form with data to transcribe the video
 */
